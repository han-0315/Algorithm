...
## 풀이설명
최소, 최대 세그먼트 트리를 구성하면 쉽게 풀린다.
먼저, segment tree 자체를 해당하는 범위의 최솟값과 최댓값을 가지게 지정한다.
자료형은 `pair<int, int>` 를 사용했고 `first = min, second = max` 로 두고 계산했다. 
트리는 다음과 같이 업데이트 된다.
`seg[i].first = min(seg[i << 1].first, seg[i << 1 | 1].first);` 
유형2의 연산, 해당 범위의 값이 맞는 지 확인할 때는 아래와 같이 범위를 파악하고 맞는 지 확인한다.
```cpp
